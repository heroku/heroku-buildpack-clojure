...
lint-scripts:
	@git ls-files -z --cached --others --exclude-standard 'bin/*' 'etc/*' 'lib/*' | grep -zv 'lib/buildpack-stdlib-v7.sh' | xargs -0 shellcheck --check-sourced --color=always

lint-ruby:
	@bundle exec rubocop

check-format:
	@shfmt -f . | grep -v "vendor/" | grep -v "opt/" | grep -v "test/spec/fixtures/" | grep -v "lib/buildpack-stdlib-v7.sh" | xargs shfmt --diff

format:
	@shfmt -f . | grep -v "vendor/" | grep -v "opt/" | grep -v "test/spec/fixtures/" | grep -v "lib/buildpack-stdlib-v7.sh" | xargs shfmt --write --list
